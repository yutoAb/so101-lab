...
`vision/ishokujuu-automation.md` の **World API 構想**における「体（物理的動作）= ロボティクス」セグメントを、自分の手元で実装可能な最小単位で検証する。
キャリア軸の **MLOps（データ収集 → 学習 → 軽量化 → リアルタイム推論ループ）** を、Speech 側（`speech-mlops-platform`）と並行して Robot 側でも回し、ループ設計の汎用性を確かめる。
...
- 2026-05-09: キット到着、so101-lab リポジトリ初期化
